                                                    ...
                                                    <table class="table table-bordered">
                                                        ...
                                                        <tbody>
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    <textarea class="form-control" name="title_en" rows="3" placeholder="Title (EN)">{{ old('title_en', is_array($photography->title) ? ($photography->title['en'] ?? '') : $photography->title) }}</textarea>
                                                                    @error('title_en') <div class="text-danger small">{{ $message }}</div> @enderror
                                                                </td>
                                                                <td>
                                                                    <textarea class="form-control" name="title_id" rows="3" placeholder="Judul (ID)">{{ old('title_id', is_array($photography->title) ? ($photography->title['id'] ?? '') : $photography->title) }}</textarea>
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    <textarea class="form-control" name="description_en" rows="3" placeholder="Description (EN)">{{ old('description_en', is_array($photography->description) ? ($photography->description['en'] ?? '') : $photography->description) }}</textarea>
                                                                    @error('description_en') <div class="text-danger small">{{ $message }}</div> @enderror
                                                                </td>
                                                                <td>
                                                                    <textarea class="form-control" name="description_id" rows="3" placeholder="Deskripsi (ID)">{{ old('description_id', is_array($photography->description) ? ($photography->description['id'] ?? '') : $photography->description) }}</textarea>
